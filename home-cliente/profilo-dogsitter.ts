...
export class ProfiloDogsitter implements OnInit {
  ...
  offertaSelezionata:  Offre | null            = null;
  formAperta           = false;
  ...
  ngOnInit(): void {
    ...
    this.dogsitterSrv.getByUsername(username).subscribe({
      next: (res: Dogsitter) => {
        this.dogsitter   = res;
  ...
  caricaServizi(username: string): void {
    this.dogsitterSrv.getServizi(username).subscribe({
      next: (res: Offre[]) => { this.offerte = res ?? []; },
      error: () => { this.offerte = []; },
    });
  }
  ...
  confermaPrenotazione(): void {
    ...
    const payload = {
      ...
      categoriaServizio: this.offertaSelezionata?.catServizio,
      prezzoListino:     this.offertaSelezionata?.prezzoListino,
      durataMinuti:      this.offertaSelezionata?.durataMinuti,   // ← nuovo campo
